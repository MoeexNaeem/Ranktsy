...
interface ConnectedShop { shopId: string; shopName: string; connectedAt: string }
...
export function MyShopTab() {
  ...
  const activeShopId = (manualShopId && shops?.some(s => s.shopId === manualShopId))
    ? manualShopId
    : (shops?.[0]?.shopId ?? null)

  const { data, isLoading, isError } = useQuery({
    queryKey: ['etsy-insights', activeShopId],
    queryFn: async () => (await axios.get(`/api/etsy/insights?shopId=${activeShopId}`)).data.data as Insights,
  ...
  const disconnect = useMutation({
    mutationFn: async (shopId: string) => (await axios.post('/api/etsy/oauth/disconnect', { shopId })).data,
    onSuccess: (_d, shopId) => {
  ...
  if (shopsLoading) return <Loading label="Checking your Etsy connections…" />
  if (shopsError) return <ErrorBox>Couldn&apos;t reach the shop-insights service. Please try again.</ErrorBox>

  // ── No shop connected → CTA ──────────────────────────────────────────────────
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        {shops.map(s => {
          const on = s.shopId === activeShopId
          return (
            <button key={s.shopId} onClick={() => setManualShopId(s.shopId)}
              style={{
                fontSize: 13, fontWeight: 500, fontFamily: 'inherit', padding: '8px 16px', borderRadius: 100, cursor: 'pointer',
                background: on ? C.orange : C.paper, color: on ? '#fff' : C.ink, border: `1px solid ${on ? C.orange : C.hairInk}`,
              }}>
              {s.shopName}
            </button>
          )
        ...
        <a href="/api/etsy/oauth/connect"
          style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 500, color: C.orange, background: 'transparent', border: `1px dashed ${C.orange}`, padding: '8px 16px', borderRadius: 100, textDecoration: 'none' }}>
          + Connect another shop
        </a>
      </div>

      {isLoading && <Loading label="Loading shop insights…" />}
      {isError && <ErrorBox>Couldn&apos;t load insights for this shop. Please try again.</ErrorBox>}

      {data?.connected && data.shop && data.summary && !isLoading && (() => {
        const s = data.shop!
          ...
          <>
            ...
            <div className="rstack-sm" style={{ display: 'flex', alignItems: 'center', gap: 16, background: C.paper, border: `1px solid ${C.hairInk}`, borderRadius: 10, padding: '16px 18px' }}>
              ...
              <div className="rfull-sm" style={{ display: 'flex', gap: 10, flexShrink: 0 }}>
                ...
                <button onClick={() => { if (activeShopId && confirm(`Disconnect ${s.shop_name}? You can reconnect anytime.`)) disconnect.mutate(activeShopId) }} disabled={disconnect.isPending}
